...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
    ...
    set<pair<int, int>> visitedPixels;
    os.Add(config.seedpoint);

    while (!os.IsEmpty()) {
        PixelPoint currentPoint = os.Remove();
        pair<int, int> currentPosition = {currentPoint.x, currentPoint.y};

        if (visitedPixels.find(currentPosition) != visitedPixels.end()) {
            continue;
        }
        visitedPixels.insert(currentPosition);

        RGBAPixel suggestedColor = (*config.picker)(currentPoint);
        //RGBAPixel* targetPixel = config.img.getPixel(currentPoint.x, currentPoint.y);

        if (suggestedColor != *config.img.getPixel(currentPoint.x, currentPoint.y)) {
            framecount++;
            *config.img.getPixel(currentPoint.x, currentPoint.y); = suggestedColor; // could be this
        ...
        if (currentPoint.y > 0) {
            RGBAPixel* Pixel = config.img.getPixel(currentPoint.x, currentPoint.y - 1);
            if (Pixel->distanceTo(config.seedpoint.color) <= config.tolerance) {
                os.Add(PixelPoint(currentPoint.x, currentPoint.y - 1));
            }
        }

        if (currentPoint.x < config.img.width() - 1) {
            RGBAPixel* Pixel = config.img.getPixel(currentPoint.x + 1, currentPoint.y);
            if (Pixel->distanceTo(config.seedpoint.color) <= config.tolerance) {
                os.Add(PixelPoint(currentPoint.x + 1, currentPoint.y));
            }
        }

        if (currentPoint.y < config.img.height() - 1) {
            RGBAPixel* Pixel = config.img.getPixel(currentPoint.x, currentPoint.y + 1);
            if (Pixel->distanceTo(config.seedpoint.color) <= config.tolerance) {
                os.Add(PixelPoint(currentPoint.x, currentPoint.y + 1));
            }
