...
#define BRAEDENFONGIMAGE "images-original/braedenfong-502x415.png"
#define BRAEDENFONGX 430 // midpoint x-coordinate for your image
#define BRAEDENFONGY 50 // midpoint y-coordinate for your image
#define BRAEDENFONGTOLERANCE 0.25 // Adjust tolerance as needed
#define BRAEDENFONGFRAMEFREQ 500
...
... [The rest of your existing file content] ...

Test case for flooding "braedenfong-502x415.png" with solid color and creating a .gif
TEST_CASE("fill::braedenfong solid color gif", "[weight=1][part=fill]") {
    PNG img;
    img.readFromFile(BRAEDENFONGIMAGE);
    SolidColorPicker solidPicker(RGBAPixel(255, 0, 0)); // Red color

    filler::FillerConfig config;
    config.img = img;
    config.seedpoint = PixelPoint(BRAEDENFONGX, BRAEDENFONGY, *img.getPixel(BRAEDENFONGX, BRAEDENFONGY));
    config.tolerance = BRAEDENFONGTOLERANCE;
    config.frameFreq = BRAEDENFONGFRAMEFREQ;
    config.picker = &solidPicker;

    animation anim = filler::FillBFS(config);
    anim.write("images-output/braedenfong_solid_color.gif");
}

Test case for flooding "braedenfong-502x415.png" with stripe pattern and creating a .gif
TEST_CASE("fill::braedenfong stripe pattern gif", "[weight=1][part=fill]") {
    PNG img;
    if (!img.readFromFile(BRAEDENFONGIMAGE)) {
        cerr << "Error loading image file: " << BRAEDENFONGIMAGE << endl;
        exit(1); // or handle the error as appropriate
    }

    if (BRAEDENFONGX >= img.width() || BRAEDENFONGY >= img.height()) {
        cerr << "Seed point is out of bounds." << endl;
        exit(1); // or handle the error as appropriate
    }

    StripeColorPicker stripePicker(40, RGBAPixel(0, 255, 0), RGBAPixel(0, 0, 255)); // Green and Blue stripes
    ...
    config.tolerance = BRAEDENFONGTOLERANCE;
    config.frameFreq = BRAEDENFONGFRAMEFREQ;
    config.picker = &stripePicker;

    animation anim = filler::FillBFS(config);
    anim.write("images-output/braedenfong_stripe_pattern.gif");
}


// Test case for flooding "braedenfong-502x415.png" with border pattern and creating a .gif
TEST_CASE("fill::braedenfong border pattern gif", "[weight=1][part=fill]") {
    PNG img;
    img.readFromFile(BRAEDENFONGIMAGE);
    RGBAPixel borderColor(255, 0, 0); // Red border color
    BorderColorPicker borderPicker(10, img, *img.getPixel(BRAEDENFONGX, BRAEDENFONGY), borderColor, BRAEDENFONGTOLERANCE); // 10 pixels border width
    ...
    config.tolerance = BRAEDENFONGTOLERANCE;
    config.frameFreq = BRAEDENFONGFRAMEFREQ;
    config.picker = &borderPicker;

    animation anim = filler::FillBFS(config);
    anim.write("images-output/braedenfong_border_pattern.gif");
}
